 ...
 class WeChatAuth {
    constructor (config) {
     let defaultConfig = {
       appid: '',
       responseType: 'code',
       scope: 'snaspi_base', // 已微信认证的服务号拥有snsapi_base和snsapi_userinfo
       component_appid: '', // 租户第三方平台的appid
       getCodeCallback: () => {}
     }
    ...
    openAuthPage (redirectUri = window.location.href) {
      ...
      let authParams = `?appid=${this.config.appid}&redirect_uri=${redirectUri}&response_type=${this.config.responseType}&scope=${this.config.scope}&component_appid=${this.config.component_appid}#wechat_redirect`
